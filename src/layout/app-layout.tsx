...
import { getChurches } from 'apiSdk/churches';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'staff-member', 'volunteer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ChurchInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/churches` : null),
    () => getChurches({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Church',
      path: isTenantUser ? `/churches/view/${data?.[0]?.id}` : '/churches',
      entity: 'church',
